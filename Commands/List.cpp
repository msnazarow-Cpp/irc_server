...
bool List::execute(Server & server, Client & client)
{
	if (RegisteredCommand::execute(server, client))
		return(true);;
	client._received_msgs.push(clientReply(Message(RPL_LISTSTART, RPL_LISTSTART_MESS),client));
	for (std::map<std::string, Channel>::iterator it = server._channels.begin(); it != server._channels.end(); it++)
		client._received_msgs.push(clientReply(Message(RPL_LIST, (*it).first + " " + SSTR((*it).second.users.size()) + " :" + (*it).second.getTopic()),client));
	client._received_msgs.push(clientReply(Message(RPL_LISTEND, RPL_LISTEND_MESS),client));
